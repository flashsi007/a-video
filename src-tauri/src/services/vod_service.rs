...
pub async fn query_videos(query: VodQuery) -> Result<QueryResult, String> {
    ...
    let count_param_refs: Vec<&dyn rusqlite::ToSql> = count_params.iter().map(|b| b.as_ref()).collect();
    let total: usize = DB_INSTANCE.conn.lock().unwrap().query_row(
        &count_sql,
        count_param_refs.as_slice(),
        |row| row.get::<_, i64>(0)
    ).map(|v| v as usize).map_err(|e| e.to_string())?;
    let videos = DB_INSTANCE.query(
        &sql,
        &param_refs,
        |row| VideoInfo::from_row(row),
